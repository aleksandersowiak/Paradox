...
?>
<h2>Edycja Historii</h2>
<form name="photo" action="page.php?id=2&edit=history" method="post">
<table align="center">
<tr><td>
...
<?php
		$zapytanie_front_page = mysql_query ("SELECT history FROM containers");
		$wynik_front_page = mysql_fetch_assoc($zapytanie_front_page);
		do{
			echo $wynik_front_page['history'];
...
if (isset($_POST['kom'])){
	...
	$w = mysql_query("UPDATE `containers` SET history='".$text."'") or die (mysql_error());
	
	echo '<p class="success">Post został zaktualiwany</p>';
	echo '<meta http-equiv="refresh" content="2;url=page.php?id=2">';
